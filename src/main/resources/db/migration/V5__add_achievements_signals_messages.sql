...
--
-- `is_active` is listed explicitly in every INSERT below (Stage 6/ADR-013) - it is NOT NULL with
-- no column DEFAULT in the AchievementDefinition entity mapping, so relying on an implicit default
-- would fail on a fresh install where V0 (Hibernate-generated) created this table.
INSERT INTO achievement_definitions (title, description, emoji, trigger_type, trigger_value_int, max_awards_per_customer, display_order, is_active, created_at, updated_at)
SELECT 'Первая покупка', 'Добро пожаловать в программу лояльности!', '🎉', 'FIRST_PURCHASE', NULL, 1, 100, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM achievement_definitions WHERE trigger_type = 'FIRST_PURCHASE');

INSERT INTO achievement_definitions (title, description, emoji, trigger_type, trigger_value_int, max_awards_per_customer, display_order, is_active, created_at, updated_at)
SELECT '5 покупок', 'Совершено 5 покупок', '⭐', 'N_PURCHASES_TOTAL', 5, 1, 90, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM achievement_definitions WHERE trigger_type = 'N_PURCHASES_TOTAL' AND trigger_value_int = 5);

INSERT INTO achievement_definitions (title, description, emoji, trigger_type, trigger_value_int, max_awards_per_customer, display_order, is_active, created_at, updated_at)
SELECT '10 покупок', 'Совершено 10 покупок', '🌟', 'N_PURCHASES_TOTAL', 10, 1, 80, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM achievement_definitions WHERE trigger_type = 'N_PURCHASES_TOTAL' AND trigger_value_int = 10);

INSERT INTO achievement_definitions (title, description, emoji, trigger_type, trigger_value_int, max_awards_per_customer, display_order, is_active, created_at, updated_at)
SELECT 'Постоянный клиент', 'Достигнут статус REGULAR', '⭐', 'BECAME_REGULAR', NULL, 1, 70, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM achievement_definitions WHERE trigger_type = 'BECAME_REGULAR');

INSERT INTO achievement_definitions (title, description, emoji, trigger_type, trigger_value_int, max_awards_per_customer, display_order, is_active, created_at, updated_at)
SELECT 'VIP клиент', 'Достигнут статус VIP', '👑', 'BECAME_VIP', NULL, 1, 60, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM achievement_definitions WHERE trigger_type = 'BECAME_VIP');

INSERT INTO achievement_definitions (title, description, emoji, trigger_type, trigger_value_int, max_awards_per_customer, display_order, is_active, created_at, updated_at)
SELECT 'Ранняя пташка', 'Покупка с 6 до 9 утра', '🌅', 'PURCHASE_TIME_WINDOW', NULL, NULL, 50, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
